(function(factory) {
	...
	var load = function(options) {
		...
		if (Facilino.profiles['processor']==='ESP32')
		{
			...
			Blockly.Arduino.definitions_['define_adv_buzzer_tone']='void _tone(int pin, float noteFrequency, long noteDuration, int silentDuration){\n_pwms[pin]->writeTone(noteFrequency);\n  delay(noteDuration);\n  _pwms[pin]->writeTone(0);\n  delay(silentDuration);\n}\n';
			...
			{
				code+='_tone('+pin+','+dropdown_freq+','+delay_time+',0);\n  delay('+delay_time+');\n';
			}
		}
		else
		{
		...
		Blockly.Arduino.dyor_piezo_buzzer_predef_sounds = function() {
			...
			if (Facilino.profiles['processor']==='ESP32')
			{
				...
				pin = pin+',';
				Blockly.Arduino.definitions_['define_adv_buzzer_tone']='void _tone(int pin, float noteFrequency, long noteDuration, int silentDuration){\n_pwms[pin]->writeTone(noteFrequency);\n  delay(noteDuration);\n  _pwms[pin]->writeTone(0);\n  delay(silentDuration);\n}\n';
				Blockly.Arduino.definitions_['define_simpleexpressions_buzzer_bendtones']='void bendTones (int pin, float initFrequency, float finalFrequency, float prop, long noteDuration, int silentDuration){\n  if(initFrequency < finalFrequency){\n	for (int i=initFrequency; i<finalFrequency; i=i*prop) {\n	  _tone(pin,i,noteDuration,silentDuration);\n	}\n  }  else{\n	for (int i=initFrequency; i>finalFrequency; i=i/prop) {\n	  _tone(pin,i,noteDuration,silentDuration);\n	}\n  }\n}\n';
			}
			else
			{
				Blockly.Arduino.definitions_['define_adv_buzzer_tone']='void _tone(int buzzerPin, float noteFrequency, long noteDuration, int silentDuration){\n  tone(buzzerPin, noteFrequency, noteDuration);\n  delay(noteDuration);\n  delay(silentDuration);\n}\n';
		...
		Blockly.Blocks.dyor_piezo_buzzer_predef_sounds = {
			...
			init: function() {
				...
				if (window.FacilinoAdvanced===true)
				{
					...
					this.appendDummyInput('').appendField(new Blockly.FieldImage(Facilino.path+'img/blocks/speaker.svg', 20*options.zoom, 20*options.zoom)).appendField(new Blockly.FieldDropdown([
				...
				{
					this.appendValueInput('PIN').appendField(new Blockly.FieldImage(Facilino.path+'img/blocks/speaker.svg', 20*options.zoom, 20*options.zoom)).appendField(new Blockly.FieldImage(Facilino.path+'img/blocks/musical-note.svg', 20*options.zoom, 20*options.zoom)).appendField(new Blockly.FieldDropdown([
							[Facilino.locales.getKey('LANG_PIEZZO_BUZZER_PREDEF_CONNECTION'), '0'],
							[Facilino.locales.getKey('LANG_PIEZZO_BUZZER_PREDEF_DISCONNECTION'), '1'],
							[Facilino.locales.getKey('LANG_PIEZZO_BUZZER_PREDEF_BUTTON_PUSHED'), '2'],
							[Facilino.locales.getKey('LANG_PIEZZO_BUZZER_PREDEF_MODE1'), '3'],
							...
							[Facilino.locales.getKey('LANG_PIEZZO_BUZZER_PREDEF_FART2'),'17'],
							[Facilino.locales.getKey('LANG_PIEZZO_BUZZER_PREDEF_FART3'),'18']
						]), 'OPTION').appendField(new Blockly.FieldImage("img/blocks/digital_signal.svg",20*options.zoom,20*options.zoom)).setCheck(['DigitalPin','PWMPin']).setAlign(Blockly.ALIGN_RIGHT);
						this.setInputsInline(true);
				}
				this.setInputsInline(false);
				this.setPreviousStatement(true,'code');
		...
		Blockly.Blocks.zum_piezo_buzzerav = {
			...
			init: function() {
				...
				{
					this.appendValueInput('PIN').appendField(new Blockly.FieldImage('img/blocks/speaker.svg',20*options.zoom,20*options.zoom)).appendField(new Blockly.FieldImage(Facilino.path+"img/blocks/digital_signal.svg", 20*options.zoom, 20*options.zoom)).setCheck(['DigitalPin','PWMPin']);
					this.appendValueInput('TONE', Number).setAlign(Blockly.ALIGN_RIGHT).appendField(new Blockly.FieldImage('img/blocks/vibration.svg', 20*options.zoom, 20*options.zoom),'INP1');
