...
<html><?php include "head.php"; ?>
	<body>
		...
		elseif  (isset($_GET["action"])&&($_GET["action"]=="import")&&isset($_POST["share_link"])&&!isset($_POST["action"])&&isset($_POST["import_button"])){
			//Import project
			$key=substr($_POST["share_link"],strpos($_POST["share_link"],"key")+4);
			$query ="SELECT id from `projects` WHERE share_key=\"".$key."\"";
			$result = mysqli_query($con,$query);
			$rows = mysqli_num_rows($result);
			if ($rows==1)
			{
		...
		elseif (isset($_GET["action"])&&($_GET["action"]=="import")&&!isset($_POST["action"])){
			...
			<form action="dashboard.php?action=import" method="POST" style="margin-bottom:0.5em">
			<input type="text" maxlength="250" size="250" name="share_link" value="" style="margin-top: 0px; padding: 0px; padding-left: 10px; font-size: 16px; width:80%"/></br><?php
