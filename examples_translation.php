...
{
	$query_examples = "SELECT `key`,`".$lang."` FROM `examples_translate` WHERE `example_id`=".$_GET["example_id"]." or `example_id`=0";
}
else
{
	$query_examples = "SELECT `key`,`".$lang."` FROM `examples_translate` WHERE 1";
}
$result_examples = mysqli_query($con,$query_examples);		
$translation=mysqli_fetch_all($result_examples,MYSQLI_ASSOC);
$examples=array();
foreach ($translation as $entry)
{
	$examples[$entry["key"]]=$entry[$lang];
}
?>